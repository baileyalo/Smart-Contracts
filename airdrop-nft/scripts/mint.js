...
const existingContractAddr = "0xE1c6E15EaCEe356AbC285B9d0A76B1810a441a00";

async function main() {
  const nft = await hre.ethers.getContractAt("MadaraTestnet", existingContractAddr);

  const signer0 = await ethers.provider.getSigner(0);
  const nonce = await signer0.getTransactionCount();
  for(let i = 0; i < friends.length; i++) {
    const tokenURI = "https://gateway.ipfs.io/ipfs/QmRhg5LHqnevogemMz7whz4V19NfzbvKADanrVnXfBd6xW";
